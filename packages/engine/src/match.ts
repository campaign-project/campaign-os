/**
 * @campaign-os/engine — record-linkage matcher.
 *
 * Does a petition signer correspond to a row in the registered-voter file? Two-stage:
 * BLOCKING (zip5 + last-name prefix) then SCORING (Jaro-Winkler name, house-number-dominant
 * address). Returns a BANDED verdict (MATCH / REVIEW / NO_MATCH) — never a naked boolean —
 * so the validity layer can refuse to auto-count an uncertain signature (the legal bright line).
 *
 * Pure + deterministic. This is the kernel the field app runs ON-DEVICE against the synced
 * per-geography voter index, and the server runs in bulk. Same code both places.
 */

import { normName, normAddress, type NormName, type NormAddress } from "./normalize";
...
/** Build the per-geography index the device matches against. */
